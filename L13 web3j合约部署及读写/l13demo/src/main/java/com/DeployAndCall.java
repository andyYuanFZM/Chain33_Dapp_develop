...
{
    public static void main(String[] args) throws IOException, InterruptedException, ExecutionException, InvalidAlgorithmParameterException, NoSuchAlgorithmException, NoSuchProviderException {
        ...
        // 读取合约二进制代码
        String contractBin = new String(Files.readAllBytes(Paths.get(System.getProperty("user.dir"),  "/Chain33_Dapp_develop/L13 web3j合约部署及读写/l13demo/src/main/resource/", "contract.code")), StandardCharsets.UTF_8);
     ...
     */
    private static String deployContract(Web3j web3, Credentials credentials, String contractBin, BigInteger chainId) throws IOException, InterruptedException, ExecutionException {
       // 获取nonce
       ...
       // 合约构造函数参数
       String encodedConstructor = FunctionEncoder.encodeConstructor(Arrays.asList(new org.web3j.abi.datatypes.Utf8String("init ERC1155")));
       BigInteger value = Convert.toWei("0", Convert.Unit.ETHER).toBigInteger(); 
       
       // 创建合约交易对象
       RawTransaction rawTransaction = RawTransaction.createContractTransaction(nonce, gasPrice, gasLimit, value, contractBin + encodedConstructor);
     ...
     * @throws ExecutionException
     */
    private static String getContractAddress(Web3j web3, String txHash) throws InterruptedException, ExecutionException {
        // 根据hash获取合约地址
     ...
     * @throws ExecutionException
     */
    private static void callContract(Web3j web3, Credentials credentials, String contractAddress, BigInteger chainId) throws IOException, InterruptedException, ExecutionException {
       // 获取nonce，交易笔数
       BigInteger nonce = web3.ethGetTransactionCount("0x4797A444f34C26e71803A1d98D5031a3cAE70650", DefaultBlockParameterName.LATEST).send().getTransactionCount();
       ...
       Function function = new Function("mint",  Arrays.asList(new Uint256(tokenId), new Uint256(supply), new Utf8String(uri)), Collections.emptyList());
     ...
     * @throws InterruptedException
     */
    private static void queryNFTInfo(Web3j web3, String contractAddress) throws InterruptedException, ExecutionException {
        // 查询账户下NFT数量
        Function function = new Function("balanceOf", Arrays.asList(new Address("0x4797A444f34C26e71803A1d98D5031a3cAE70650"), new Uint256(10000)), Collections.emptyList());
        String encodedFunction = FunctionEncoder.encode(function);
        EthCall response = null;
        response = web3.ethCall(Transaction.createEthCallTransaction("0x4797A444f34C26e71803A1d98D5031a3cAE70650", contractAddress, encodedFunction), DefaultBlockParameterName.LATEST).sendAsync().get();
